...
Процедура ОбновитьТЗ()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВнутреннееПотреблениеТоваровТовары.Ссылка = &ВнутреннееПотреблениеТоваров
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Упаковка
	...
	Запрос.УстановитьПараметр("Отбор", 			Отбор);
	Запрос.УстановитьПараметр("ВнутреннееПотреблениеТоваров", 	ВнутреннееПотреблениеТоваров);
...
Функция ПолучитьСтрокуТаблицыДанныхПоДиапазону(Номенклатура)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВнутреннееПотреблениеТоваровТовары.Ссылка = &ВнутреннееПотреблениеТоваров
		|	И ВнутреннееПотреблениеТоваровТовары.Номенклатура = &Номенклатура
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Упаковка
...
Процедура ОбработатьШтрихкод(Штрихкод)
	...
	ОчиститьПоследнийВвод();
	Если НЕ ИспользоватьДиапазон Тогда
...
Процедура ДобавитьКоличествоПоДиапазону(СтрокаТЧ, КоличествоВведено)
	...
	//пользователь выбирает упаковку сам в которую упаковать по коробочно соответсвенно коеффициенту упаковки
...
Процедура  ЗавершитьОтбор()
	...
	РасхОрдер.Статус = ?(РасхожденияСуществуют, Перечисления.СтатусыРасходныхОрдеров.КОтбору, Перечисления.СтатусыРасходныхОрдеров.Отгружен);
	РасхОрдер.Записать(РежимЗаписиДокумента.Проведение);
	
	
	//ЗаказНаВнутреннееПотребление	22.07.2020
	ДокЗаказНаВнутреннееПотребление = ЗаказНаВнутреннееПотребление.ПолучитьОбъект();
	
	
	ДокЗаказНаВнутреннееПотребление.Работа = Справочники.СписокВыполняемыхРабот.НайтиПоКоду("000000021");	//Перемещение Питер
	
	//Комплектация СПБ
	РСК_УчетРабот.ЗаписатьРаботу(Истина, ЗаказНаВнутреннееПотребление.Ссылка, Справочники.РСК_ВыполняемыеРаботы.КомплектацияСПБ, Пользователь.ФизическоеЛицо, ОтборДок.Ссылка.ДатаПервогоСканирования, ОтборДок.Ссылка.ДатаПоследнегоСканирования);
	
	//ДокЗаказНаВнутреннееПотребление.Проверил = Пользователь.ФизическоеЛицо;
	//ДокЗаказНаВнутреннееПотребление.Дата1Проверки = ОтборДок.Ссылка.ДатаПервогоСканирования;
	//ДокЗаказНаВнутреннееПотребление.Дата2Проверки = ОтборДок.Ссылка.ДатаПоследнегоСканирования;
	
	ДокЗаказНаВнутреннееПотребление.ПостМесто = ПостМесто;
	ДокЗаказНаВнутреннееПотребление.Записать(РежимЗаписиДокумента.Запись);
